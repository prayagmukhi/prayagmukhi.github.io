  ...
  var initialize = function (options) {
    ...
    function eP(elem){
      if(!config.noMenu){
        ...
        if( getOffset(elem).top < root.scrollTop + config.offset + 2 ){
          return elem;
        }
      }
    ...
    while (i--) {
      if( document.getElementById(/[^#]+$/.exec(links[i].href)[0])){
        ...
        links[i].addEventListener("click", function(e) {
          ...
          var endPos = config.noMenu ? document.getElementById(hash).getBoundingClientRect().top - (config.offset) : document.getElementById(hash).getBoundingClientRect().top - (tMenu.offsetHeight + config.offset);
    ...
    var onScrollFn = function (event) {
      ...
      if(config.hideOnScroll){
        if(Math.abs(lastScrollTop - scrollY()) <= 50) {
          return;
        }

        if (scrollY() > lastScrollTop ){
          // Hide menu
          if(typeof config.hideFn == "function" && latestHideAction != 'hide') {
            latestHideAction = 'hide';
            config.hideFn();
          }
          tMenu.classList.add('nav-hide');
          tMenu.classList.remove('nav-show')
        } else {
          // Show menu
          if(typeof config.showFn == "function" && latestHideAction != 'show') {
            latestHideAction = 'show';
            config.showFn();
          }
          if(scrollY() + getViewportH() < root.scrollHeight) {
              tMenu.classList.remove('nav-hide');
              tMenu.classList.add('nav-show');
          }
        }

        if( (getViewportH() + scrollY() ) > (root.scrollHeight - 100) ){
          tMenu.classList.remove('nav-hide');
          tMenu.classList.add('nav-show');
        }
